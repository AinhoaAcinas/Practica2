...
gdispersion <- grafico1+geom_point()
gdispersion

grafico2 <- ggplot(ds,aes(x=MinTemp, y = MaxTemp, colour=RainTomorrow))+geom_point()
grafico2

grafico3 <- grafico2+ggtitle("Dispersión temperatura máxima y minima")
grafico3
...
gdis1 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am))+geom_point()
gdis1

gdis2 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, shape = RainToday))+geom_point()
gdis2

gdis3 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, colour = RainToday))+geom_point() #Se observa mucho mejor con el color
gdis3

gtitulo <- gdis3 + ggtitle("Grafico de dispersion entre temperatura y presion")
...
str(iris) #Contiene 5 variables y 150 objetos

iris.grafico1 <- ggplot(iris.tidy, aes(x = Species, y = Value))
iris.grafico1

iris.grafico1+geom_point()+ggtitle("Grafico valor y especies de iris")

iris.grafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part))
iris.grafico2 + geom_point()

horizontal <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part))+geom_point() + facet_grid(Measure~.)
horizontal
vertical <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part))+geom_point() + facet_grid(.~Measure) #Se puede comparar mejor con cortes verticales
vertical

iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part))+ geom_jitter() #La diferencia esta en que mediante geom_jitter los puntos salen mas dispersos y mediante geom_ponint los puntos salen especificamente en las coordenadas que le corresponenen.
iris.grafico3
...
str(DatosEjer4) #Tiene 3 variables y 200 registros
grafico1 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour=Fabricante))+geom_point()
grafico1 #Podemos observar que no es muy adecuado para ver la evolucion ya que apesar de que a groso modo si que se puede comprobar su evolucion no queda del todo claro. Seria mas apropiado crear una linea que una los puntos para ver mas detalladamente su trayectoria de ventas a lo largo del tiempo.

glinea <- grafico1 + geom_line()
glinea

glinea2 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour = Fabricante))+geom_line(aes(linetype=Fabricante))+geom_point()
glinea2

gtitulo <- glinea2 + ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
gtitulo
...
grafico1 <- ggplot(Experimento, aes(x = Dosis, y = Altura,group = Complemento, shape = Complemento, linetype = Complemento)) + geom_point()+ geom_line()
grafico1

gberror <- ggplot(Experimento, aes(x = Dosis, y = Altura,group = Complemento, shape = Complemento, linetype = Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width =.1, position = position_dodge(0.05)) + geom_point()+ geom_line() + theme_classic()
gberror

gberror2 <- ggplot(Experimento, aes(x = Dosis, y = Altura),colour = "Blue") + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width =.1, position = position_dodge(0.05)) + geom_point(aes(shape = Complemento))+ geom_line(aes(linetype = Complemento), scale_colour = "Red") + theme_classic()
gberror2
...
gbaxplot <- ggplot(ds, aes(x=Year, y = MaxTemp)) + geom_boxplot()
gbaxplot

gviolin <- ggplot(ds, aes(x=Year, y = MaxTemp, fill = Year)) + geom_violin()
gtitulos <- gviolin + labs(title = "Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura")
gtitulos

gfinal <- gtitulos + theme(axis.text.x=element_text(angle=45, hjust=1), legend.position = "none")
gfinal
...
file7 <- read.table("http://assets.datacamp.com/blog_assets/chol.txt", header = T)
file7

gbase <- ggplot(data = file7, aes(x= HEIGHT)) + geom_histogram()
gbase

gtitulos <- gbase + labs(title="Histograma de la variable Altura", x = "Altura(cm)", y = "Frecuencia")
gtitulos

grango <- ggplot(data = file7, aes(x= HEIGHT)) + geom_histogram(breaks = seq(155,195, by = 2), bins = 20) + labs(title="Histograma de la variable Altura", x = "Altura(cm)", y = "Frecuencia")
grango

gcolor <- ggplot(data = file7, aes(x= HEIGHT)) + geom_histogram(breaks = seq(155,195, by = 2), bins = 20, fill = "Blue", color = "Red", alpha = 0.5) + labs(title="Histograma de la variable Altura", x = "Altura(cm)", y = "Frecuencia")
gcolor

gcurva <- ggplot(data = file7, aes(x= HEIGHT)) + geom_histogram(aes(y=stat(density)),breaks = seq(155,195, by = 2), bins = 20, fill = "Blue", color = "Red", alpha = 0.5) + geom_density(col="green") + labs(title="Histograma de la variable Altura", x = "Altura(cm)", y = "Frecuencia")
gcurva

gcurva <- ggplot(data = file7, aes(x= HEIGHT)) + geom_histogram(aes(y=stat(density)),breaks = seq(155,195, by = 2), bins = 20, fill = "Blue", color = "Red", alpha = 0.5) + geom_density(col="green") + labs(title="Histograma de la variable Altura", x = "Altura(cm)", y = "Frecuencia")+facet_grid(.~SMOKE)
gcurva
